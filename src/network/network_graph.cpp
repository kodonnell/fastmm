...
NetworkGraph::NetworkGraph(const Network &network_arg, TransitionMode mode) : network(network_arg)
{
  ...
  for (int i = 0; i < N; ++i)
  {
    ...
    if (mode == TransitionMode::FASTEST)
    {
      ...
      {
        throw std::invalid_argument("FASTEST mode requires speed on all edges. Edge " +
                                    std::to_string(edge.id) + " is missing speed.");
      }
    }
    else
    {
      // Distance-based (SHORTEST)
      g[e].cost = edge.length;
    }
  }
...
void NetworkGraph::print_graph() const
{
  ...
  for (boost::tie(it, end) = boost::edges(g); it != end; ++it)
    std::cout << " index " << g[*it].index << " edge " << network.get_edge_id(g[*it].index) << " "
    ...
    NodeIndex source, NodeIndex target) const
{
  ...
  while (!Q.empty())
  {
         ...
         out_i != out_end; ++out_i)
    {
      ...
      temp_cost = dmap.at(u) + g[e].cost;
      auto iter = dmap.find(v);
      if (iter != dmap.end())
      {
        // dmap contains node v
        if (iter->second > temp_cost)
        {
          // a smaller distance is found for v
          pmap[v] = u;
          dmap[v] = temp_cost;
          Q.decrease_key(v, temp_cost);
        }
      }
      else
      {
         ...
         out_i != out_end; ++out_i)
    {
      ...
      if (iter != dmap.end())
      {
        ...
        if (iter->second > temp_cost)
        {
          ...
          if (Q.contain_node(v))
          {
            Q.decrease_key(v, temp_cost + h);
          }
          else
          {
            Q.push(v, temp_cost + h);
          }
        }
      }
      else
                                 ...
                                 double cost) const
{
       ...
       out_i != out_end; ++out_i)
  {
    ...
    //              source, boost::target(e, g), g[e].cost);
    ...
    NodeIndex source, NodeIndex target, double *cost) const
{
       ...
       out_i != out_end; ++out_i)
  {
    ...
    SPDLOG_TRACE("  Check Edge from {} to {} cost {}",
                 source, boost::target(e, g), g[e].cost);
    if (target == boost::target(e, g) &&
        g[e].cost <= current_cost)
    {
      current_cost = g[e].cost;
      *cost = current_cost;
      result = g[e].index;
    }
  }
